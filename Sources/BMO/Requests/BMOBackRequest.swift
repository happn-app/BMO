...
public enum BMOBackRequestPart<DbObjectType, DbFetchRequestType, DbAdditionalInfoType> {
...
public protocol BMOBackRequest {
	...
	func backRequestParts() throws -> [RequestPartId: BMOBackRequestPart<DbType.ObjectType, DbType.FetchRequestType, AdditionalDbRequestInfoType>]
	...
	func backResultsImporter<BridgeType : BMOBridge>(for requestPartId: RequestPartId) -> AnyBMOBackResultsImporter<BridgeType>? where BridgeType.DbType == DbType, BridgeType.AdditionalDbRequestInfoType == AdditionalDbRequestInfoType */
	...
	Will always be called on the context. */
	func processBridgeError(_: Error)
	
	/* ********* */
	
	/** Return `nil` if the results of the request part should not be processed. */
	func dbForImportingResults(ofRequestPart requestPart: BMOBackRequestPart<DbType.ObjectType, DbType.FetchRequestType, AdditionalDbRequestInfoType>, withId id: RequestPartId) -> DbType?
	...
	Will always be called on the context. */
	func prepareResultsImport(ofRequestPart requestPart: BMOBackRequestPart<DbType.ObjectType, DbType.FetchRequestType, AdditionalDbRequestInfoType>, withId id: RequestPartId, inDb db: DbType) throws -> Bool
	
	/** Perform here the actions to finish the import of the data for the given
	request part.
	
	Will always be called on the context. */
	func endResultsImport(ofRequestPart requestPart: BMOBackRequestPart<DbType.ObjectType, DbType.FetchRequestType, AdditionalDbRequestInfoType>, withId id: RequestPartId, inDb db: DbType, importResults: BMOImportResult<DbType>) throws
	...
	**Not** called if `prepareResultsImport...` returns `false` though.
	...
	Will always be called on the context. */
	func processResultsImportError(ofRequestPart requestPart: BMOBackRequestPart<DbType.ObjectType, DbType.FetchRequestType, AdditionalDbRequestInfoType>, withId id: RequestPartId, inDb db: DbType, error: Error)
